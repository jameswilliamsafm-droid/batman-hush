...
export function OrderCard({ order, onControl, onClone, controlBusy }: OrderCardProps) {
  ...
  // 🔥 FIX: Build chart from actual run data since cumulativeViews may be 0
  const plannedData = useMemo(() => {
    const runs = order.runs || [];
    if (runs.length === 0) return [];

    // Try cumulativeViews first
    ...
    // 🔥 Fallback: build cumulative from individual run values
    ...
    <article className="rounded-2xl border border-yellow-500/20 bg-gradient-to-br from-gray-900 to-black p-3 sm:p-5">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-start sm:justify-between sm:gap-4">
        ...
        <div className="flex flex-row flex-wrap gap-x-4 gap-y-1 sm:flex-col sm:space-y-1.5 sm:text-right">
          ...
          <p className="text-xs sm:text-sm text-gray-500">
            Quantity: <span className="font-semibold text-gray-300">{order.totalViews}</span>
          </p>
          <p className="text-xs sm:text-sm text-gray-500">
            Status:{" "}
          ...
          {order.errorMessage && (
            <p className="text-[10px] sm:text-xs text-red-400">Error: {order.errorMessage}</p>
          )}
          {finishTime && (
              ...
              >
                ...
                <Tooltip
                  ...
                  formatter={(value: number, name: string) => {
                    if (String(name).startsWith("planned")) return [null as any, null as any];
